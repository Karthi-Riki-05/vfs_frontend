...
function AcceptInvitationContent() {
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      // Pro-app invites: backend signals via verifyInvite that lifetime Pro
      // is required. Forward the user to the Pro-purchase flow with the
      // invite token so the webhook can auto-accept after payment.
      if (invitation?.requiresProPurchase) {
        window.location.href = `/invite/pro-purchase?token=${token}`;
        return;
      }
      ...
      // 402 PRO_REQUIRED → hand off to the Pro purchase page.
      if (err?.response?.data?.error?.code === "PRO_REQUIRED") {
        window.location.href = `/invite/pro-purchase?token=${token}`;
        return;
      }
      const msg =
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <TeamOutlined
            style={{ fontSize: 40, color: "#fff", marginBottom: 12 }}
          />
        ...
        <div style={{ padding: "32px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
            ...
            <Paragraph style={{ color: "#555", fontSize: 15, margin: 0 }}>
              ...
              {inviterEmail && <span> ({inviterEmail})</span>} invited you to
              join
            </Paragraph>
          ...
          >
            <Space direction="vertical" size={4} style={{ width: "100%" }}>
              ...
              <div>
                ...
                <div>
                  <Tag
                    color={invitation?.appContext === "pro" ? "gold" : "green"}
                  >
                    {appName}
                  </Tag>
                </div>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <Space direction="vertical" size={12} style={{ width: "100%" }}>
                ...
                <Button
                  ...
                  block
                  onClick={() =>
                    (window.location.href = `/register?callbackUrl=${redirectUrl}`)
                  }
                  style={{
                    borderRadius: 8,
                    height: 44,
