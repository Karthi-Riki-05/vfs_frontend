...
/* ---- local atoms (per DESIGN.md preflight-off rule) ---- */
...
// Exact-match overrides (real backend types). Unmatched types fall through to
// the prefix rules in styleFor(), then DEFAULT_STYLE.
const TYPE_STYLES: Record<string, TypeStyle> = {
  // flow pack / add-on lifecycle
...
// Prefix fallbacks so unmapped variants still get the right family icon/colour.
              ...
              {g.items.map((n) => {
                ...
                return (
                  <button
                    ...
                    className={`${RESET} w-full text-left flex items-start gap-3 p-3 rounded-2xl border border-border mb-2 transition-colors hover:bg-secondary/40 ${
